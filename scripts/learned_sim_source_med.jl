...
_dict = @strdict
basepath = "/nethome/mlouboutin3/OneShotImaging/"
plot_path = "$(basepath)/plots/$(sim_name)"
save_path = "$(basepath)/data/$(sim_name)"
data_path = "$(basepath)/data"
shot_path = "$(basepath)/over-shots"
...
for e in 1:n_epochs
    ...
    for (k, idx) in enumerate(idx_train)
        ...
        mod(k, plot_every) == 0 && plot_prediction(net, Ji, m0j, dmj, d_obs, k, e, plot_path;lr=1, n_epochs=1, name="train")
        # Testing
        if mod(k-1, test_every) == 0
            ...
            mod(k, plot_every) == 0 && plot_prediction(net, Jit, m0t, dmjt, d_obst, k, e, plot_path;lr=1, n_epochs=1, name="test")
        end

        mod(k, plot_every) == 0 && plot_losses(train_loss_h, test_loss_h, k, e, plot_path; lr=1, n_epochs=1)
